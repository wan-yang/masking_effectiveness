...
out.wkly <- lapply(files, function(x) {
  
  tmp = try(load(x), silent = T)
  
  if(any(class(tmp) == 'try-error')){
    tmp = try(res$res.wkly, silent = T)
    if(any(class(tmp) == 'try-error')){
      print(x)
      d = NULL
    }
    ...
    d = res$res.wkly %>% # status at each week
      rename(., value = perc)
  ...
  (function(d) d[, list(mean = round(mean(value, na.rm = T), 3), 
                 ...
                 by = c(key.parms,
                        'week.ending','status')]) 
save(out.wkly, file = paste0(dir_out, 'out.wkly.RData'))
print('done reading out.wkly')
