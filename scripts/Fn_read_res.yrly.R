...
out.yrly <- lapply(files, function(x) {
  
  tmp = try(load(x), silent = T)
  
  if(any(class(tmp) == 'try-error')){
    tmp = try(res$res.yrly, silent = T)
    if(any(class(tmp) == 'try-error')){
      print(x)
      d = NULL
    }
    ...
    d = res$res.yrly %>% # yearly tallies
      reshape2::melt(., id.vars = c('year','ens'))
  ...
  (function(d) d[, list(mean = round(mean(value, na.rm = T), 3), 
                 ...
                 by = c(key.parms,
                        'year', 'variable')])
save(out.yrly, file = paste0(dir_out, 'out.yrly.RData'))
print('done reading out.yrly')
