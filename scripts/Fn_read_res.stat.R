...
out.tot <-  lapply(files, function(x) {
  ...
  if(any(class(tmp) == 'try-error')){
    ...
    d = res$res.stat %>% # cumulative stats by end of the simulation
      reshape2::melt(., id.vars = c('mask.grp','age.grp'))
  ...
  (function(d) d[, list(mean = round(mean(value, na.rm = T), 3), 
                 ...
                 by = c(key.parms,
                        'mask.grp', 'age.grp', 'variable')]) # %>%
save(out.tot, file = paste0(dir_out, 'out.tot.RData'))
print('done reading out.tot')
