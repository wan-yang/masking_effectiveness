...
out.by.yr <-  lapply(files, function(x) {
  
  tmp = try(load(x), silent = T)
  
  if(any(class(tmp) == 'try-error')){
    tmp = try(res$res.stat, silent = T)
    if(any(class(tmp) == 'try-error')){
      print(x)
      d = NULL
    }
    ...
    d = res$res.byyr.stat %>% # both yearly, and cumulative stats
  ...
  (function(d) d[, list(mean = round(mean(value, na.rm = T), 3), 
                 ...
                 by = c(key.parms,
                        'year', 'timeframe', 'mask.grp', 'age.grp', 'variable')]) # %>%

save(out.by.yr, file = paste0(dir_out, 'out.by.yr.RData'))
print('done reading out.by.yr')