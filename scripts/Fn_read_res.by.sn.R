...
# read and combine model runs

out.by.sn <-  lapply(files, function(x) {
  
  tmp = try(load(x), silent = T)
  
  if(any(class(tmp) == 'try-error')){
    tmp = try(res$res.bysn.stat, silent = T)
    if(any(class(tmp) == 'try-error')){
      print(x)
      d = NULL
    }
    ...
    d = res$res.bysn.stat %>% # both season, and cumulative stats
  ...
  (function(d) d[, list(mean = round(mean(value, na.rm = T), 3), 
                 ...
                 by = c(key.parms,
                        'season', 'timeframe', 'mask.grp', 'age.grp', 'variable')]) # %>%

save(out.by.sn, file = paste0(dir_out, 'out.by.sn.RData'))
print('done reading out.by.sn')