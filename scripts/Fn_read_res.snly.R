...
out.snly <- lapply(files, function(x) {
  
  tmp = try(load(x), silent = T)
  
  if(any(class(tmp) == 'try-error')){
    tmp = try(res$res.snly, silent = T)
    if(any(class(tmp) == 'try-error')){
      print(x)
      d = NULL
    }
    ...
    d = res$res.snly %>% # seasonality tallies
      reshape2::melt(., id.vars = c('season','ens'))
  ...
  (function(d) d[, list(mean = round(mean(value, na.rm = T), 3), 
                 ...
                 by = c(key.parms,
                        'season', 'variable')])
save(out.snly, file = paste0(dir_out, 'out.snly.RData'))
print('done reading out.snly')
